<?php
/**
 * Faraa Tourism Tools — admin page that cleans Hostinger AI / Elementor data from
 * key pages and assigns the proper Faraa Tourism page templates so the theme's
...
/**
 * Register the admin page under Appearance → Faraa Tourism Tools.
 */
function laxora_tools_menu() {
    add_theme_page(
        __( 'Faraa Tourism Tools', 'laxora' ),
        __( 'Faraa Tourism Tools', 'laxora' ),
        'manage_options',
        'laxora-tools',
...
function laxora_tools_admin_notice() {
    ...
    <div class="notice notice-warning is-dismissible" data-laxora-reset-notice>
        <p>
            <strong><?php esc_html_e( 'Faraa Tourism — Cleanup recommended', 'laxora' ); ?></strong><br>
            <?php
            /* translators: %d: number of pages with Hostinger AI / Elementor data */
            printf(
                esc_html__( 'We detected Hostinger AI or Elementor data on %d page(s). Reset them to the curated Faraa Tourism design with one click.', 'laxora' ),
                absint( $count )
            );
            ?>
        </p>
        <p>
            <a href="<?php echo $url; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="button button-primary">
                <?php esc_html_e( 'Open Faraa Tourism Tools', 'laxora' ); ?>
            </a>
        </p>
    </div>
    <?php
...
/**
 * The list of "core" Faraa Tourism pages and the page template that should be assigned.
...
function laxora_tools_core_pages() {
    return array(
        ...
        'airport-transfers' => array( 'title' => __( 'Airport Transfers', 'laxora' ), 'template' => 'template-service-airport.php' ),
        'business-meetings' => array( 'title' => __( 'Business Meetings', 'laxora' ), 'template' => 'template-service-business.php' ),
        'wedding-transfers' => array( 'title' => __( 'Wedding Transfers', 'laxora' ), 'template' => 'template-service-wedding.php' ),
...
function laxora_tools_run_reset( $also_create_menu = true ) {
    ...
    if ( $also_create_menu ) {
        $menu_name = 'Faraa Tourism Primary';
...
function laxora_tools_render_page() {
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Faraa Tourism Tools', 'laxora' ); ?></h1>
        <p style="max-width:760px;">
            <?php esc_html_e( 'This one-click tool wipes any Hostinger AI / Elementor data from your five core pages (Home, About, Services, Fleet, Contact), assigns the correct Faraa Tourism page templates, makes sure the theme\'s header and footer render everywhere, and (optionally) builds a primary navigation menu pointing to all five pages.', 'laxora' ); ?>
        </p>
        ...
        <?php if ( $report ) : ?>
            <div class="notice notice-success" style="margin-top:20px;">
                ...
                <?php if ( ! empty( $report['cleaned'] ) ) : ?>
                    <p><?php esc_html_e( 'Pages reset to Faraa Tourism design:', 'laxora' ); ?> <code><?php echo esc_html( implode( ', ', $report['cleaned'] ) ); ?></code></p>
                <?php endif; ?>
                <p><?php esc_html_e( 'Visit your homepage now — the Faraa Tourism landing is back.', 'laxora' ); ?></p>
            </div>
        <?php endif; ?>
        ...
        <form method="post" onsubmit="return confirm('<?php echo esc_js( __( 'This will delete Hostinger AI / Elementor data from your core pages and assign the Faraa Tourism design. Continue?', 'laxora' ) ); ?>');">
            ...
            <p>
                <button type="submit" name="laxora_run_reset" class="button button-primary button-hero">
                    <?php esc_html_e( 'Reset to Faraa Tourism Design', 'laxora' ); ?>
                </button>
            </p>
            <p class="description" style="max-width:560px;">
                <?php esc_html_e( 'Safe to re-run any time. This does NOT delete the pages themselves — it only clears their content + Elementor data and re-assigns the correct Faraa Tourism template.', 'laxora' ); ?>
            </p>
        </form>
    </div>
    <?php
